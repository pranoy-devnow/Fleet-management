...
User management for `/internal/roles`: a searchable Medela directory, Admin-only role assignment, and access-request review.
...
Requests and Medela users are two sections behind one `SegmentedControl`, and exactly one is on screen at a time. Requests appears only for viewers holding `requests:review`, and is where an admin lands, since the queue is waiting on them. Its tab label carries the pending count.
...
- `listMedelaUsers` and `listAccessRequests` return sorted copies, so callers can hold them in state.
- `searchMedelaUsers` — free-text narrowing, blank query returns everyone
...
Only Medela Internal users are in this directory. Their roles can be reassigned by an Admin.
...
- The header badge uses `roleNotificationCount`: only an Admin with a non-empty queue sees it. That is why IT never gets a red mark they cannot act on.
...
- Switching sections clears the query. A hidden filter on arrival reads as an empty list.
- `AccessRequestList` deliberately has no heading and reuses the directory panel's frame, so the two sections are visually interchangeable. The switch is the only thing that names them.
