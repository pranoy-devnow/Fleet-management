...
describe("roleNotificationCount", () => {
  it("shows the pending count to an Admin", () => {
    expect(roleNotificationCount("admin", 3)).toBe(3);
  });

  it("hides the badge when the queue is empty", () => {
    expect(roleNotificationCount("admin", 0)).toBe(0);
  });

  it("hides the badge from IT, who cannot review requests", () => {
    expect(roleNotificationCount("it", 3)).toBe(0);
  });

  it("hides the badge when the viewer is not in the directory", () => {
    expect(roleNotificationCount(null, 3)).toBe(0);
  });
});
