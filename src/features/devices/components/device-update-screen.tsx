    ...
    <AppShell title="Device Detail & Update">
      ...
      <div className="flex max-w-xl flex-col gap-5">
          ...
          <div className="flex flex-col gap-2">
            {UPDATE_HISTORY.map((entry) => (
              <div key={entry.version} className="flex items-center justify-between border-b border-muted py-1.5 text-sm last:border-0">
                ...
                <span className="rounded-full border border-status-updated/25 bg-status-updated-tint px-2 py-0.5 text-xs text-status-updated">{entry.status}</span>
              </div>
            ))}
          </div>
        </Panel>
        ...
        <div className="flex gap-3">
          <PrimaryActionButton render={<Link href="/biomed" />} nativeButton={false}>
            Start Update
          </PrimaryActionButton>
          <SecondaryActionButton render={<Link href="/biomed" />} nativeButton={false}>
            <RefreshCw size={14} />
            Retry Update
