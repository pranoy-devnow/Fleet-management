...
export function MapHoverPanel({ device }: { device: WorldDevice | null }) {
  if (!device) {
    return (
      <div className="text-center">
        ...
        <p className="text-xs leading-relaxed text-muted-foreground">Hover a pin to see device details</p>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-2 text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">Device</div>
      ...
      <div className="mt-3 flex flex-col gap-1">
        ...
        <div className="mt-1 text-xs text-muted-foreground">FW: {device.firmware}</div>
