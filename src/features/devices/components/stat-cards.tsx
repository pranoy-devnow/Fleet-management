    ...
    <div className="mb-4 grid shrink-0 grid-cols-3 gap-4">
      {CARDS.map((card) => (
        <Link key={card.label} href={card.href} className="group text-left">
          <Panel className="h-full p-4 transition-all duration-200 group-hover:-translate-y-px group-hover:shadow-[0_2px_8px_rgba(0,0,0,0.06),0_12px_32px_rgba(0,0,0,0.06)]">
            <div className="flex items-start justify-between gap-3">
              <div className={`text-3xl font-semibold tracking-tight ${card.color}`}>{card.value}</div>
